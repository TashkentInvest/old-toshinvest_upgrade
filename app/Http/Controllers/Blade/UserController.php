...
class UserController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        // First, find the user instance
        $user = User::find($id);

        if (!$user) {
            // Handle the case where the user is not found
            message_set("User not found", 'error', 5);
            return redirect()->back();
        }

        // Check if the user to be deleted is a Super Admin and if the current user has permission to delete it
        if ($user->hasRole('Super Admin') && !auth()->user()->hasRole('Super Admin')) {
            message_set("У вас нет разрешения на редактирование администратора", 'error', 5);
            return redirect()->back();
        }
        ...
        // Clean up associated roles and permissions
