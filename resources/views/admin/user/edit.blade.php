...
@section('title', __('global.edit') . ' - ' . __('cruds.user.title'))
...
<div class="admin-page-header">
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
        <div>
            <h1 class="admin-page-title">@lang('global.edit') @lang('cruds.user.title_singular')</h1>
            <p class="admin-page-subtitle">{{ $user->name }} - foydalanuvchi ma'lumotlarini tahrirlash</p>
        </div>
...
<div class="admin-card" style="max-width: 800px;">
    <div class="admin-card-header">
        <h3 class="admin-card-title"><i class="fas fa-user-edit"></i> Foydalanuvchi ma'lumotlari</h3>
    </div>
    <div class="admin-card-body">
        <form action="{{ route('userUpdate', $user->id) }}" method="POST">
            ...
            <div class="gov-form-group">
                ...
                <input type="text" name="name" class="gov-input" value="{{ old('name', $user->name) }}" placeholder="@lang('cruds.user.fields.name')" required>
                @error('name')
            ...
            <div class="gov-form-group">
                ...
                <input type="email" name="email" class="gov-input" value="{{ old('email', $user->email) }}" placeholder="@lang('cruds.user.fields.email')" required>
                @error('email')
                <span class="gov-error-message">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="gov-form-group">
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 12px; padding: 16px; background: var(--gov-bg-light); border-radius: var(--gov-radius); border: 1px solid var(--gov-border);">
                    ...
                    <label style="display: flex; align-items: center; gap: 8px; padding: 8px 14px; background: white; border-radius: var(--gov-radius-sm); border: 1px solid var(--gov-border); cursor: pointer; transition: var(--gov-transition);">
                        <input type="checkbox" name="roles[]" value="{{ $role->name }}" {{ $user->hasRole($role->name) ? 'checked' : '' }} style="width: 18px; height: 18px; accent-color: var(--gov-primary);">
            ...
            <!-- Password Section -->
            <div class="gov-info-box" style="margin-bottom: 20px;">
                <i class="fas fa-info-circle"></i>
                <span>Parolni o'zgartirmasangiz, ushbu maydonlarni bo'sh qoldiring</span>
            </div>

            <div class="gov-form-row">
                <div class="gov-form-group">
                    <label class="gov-label">@lang('cruds.user.fields.password')</label>
                    <div style="position: relative;">
                        <input type="password" name="password" id="password" class="gov-input" placeholder="Yangi parol" style="padding-right: 50px;">
                ...
                <div class="gov-form-group">
                    <label class="gov-label">@lang('global.login_password_confirmation')</label>
                    <div style="position: relative;">
                        <input type="password" name="password_confirmation" id="password_confirmation" class="gov-input" placeholder="Parolni tasdiqlang" style="padding-right: 50px;">
